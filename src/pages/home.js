...
const Home = ({home, loading, dispatch, query}) => {
    const Router = useRouter();

    useEffect(() => {
        // console.log("[1, 2, 3]", [1, 2, 3].map(parseInt))
...
Home.getInitialProps = async ({req, pathname, query, isServer, store}) => {
...
export default WithDva(state => ( {home: state.home, loading: state.loading.effects['home/init']} ))(Home);
